...
parameters = {'C':  C_candidates,'gamma':gamma_candidates}
...
gcv_clf = GridSearchCV(basic_clf, parameters,cv=5,n_jobs=-1,iid=False,
                   scoring='accuracy')
gcv_clf.fit(X, y)
print("Based on GridSearchCV, the best C is {} and best gamma is {}".format(
        gcv_clf.best_params_['C'],
        gcv_clf.best_params_['gamma']))
